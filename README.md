🚀 Обо мне
Начал программировать в 2023 году с языка C#, но быстро перешёл во фронтенд благодаря личному ментору — учителю информатики, практикующему разработчику. Уже через несколько месяцев выступал с собственными проектами на научных конкурсах и хакатонах.

Что важно:
✔️ быстро учусь
✔️ работаю в команде (в том числе как лидер)
✔️ не боюсь сложных задач
✔️ всегда довожу проект до живого демо

🛠️ Технологии и Инструменты
https://img.shields.io/badge/-TypeScript-3178C6?logo=typescript&logoColor=white
https://img.shields.io/badge/-Next.js-000000?logo=nextdotjs&logoColor=white
https://img.shields.io/badge/-React-61DAFB?logo=react&logoColor=black
https://img.shields.io/badge/-NestJS-E0234E?logo=nestjs&logoColor=white
https://img.shields.io/badge/-Node.js-339933?logo=nodedotjs&logoColor=white
https://img.shields.io/badge/-PostgreSQL-4169E1?logo=postgresql&logoColor=white
https://img.shields.io/badge/-TailwindCSS-06B6D4?logo=tailwindcss&logoColor=white
https://img.shields.io/badge/-Prisma-2D3748?logo=prisma&logoColor=white

🤝 Софт скиллы
Лидерский опыт — руководил командой на 3 хакатонах

Командная работа — быстро налаживаю контакт и распределяю задачи

Организационные навыки — ставлю цели, дедлайны и достигаю результата

❤️ Любимые проекты
(этот блок у тебя пока не заполнен — вот пример, как его красиво оформить)

📘 ProBlog
Next.js, React, TypeScript, PostgreSQL, Prisma, Vercel
Социальная сеть с постами, авторизацией и профилями.
🏆 Участие в конкурсе «100 идей для Беларуси»

🧠 Финансовая грамотность
TypeScript, React, Next.js, PostgreSQL, SCSS
Платформа с тестом и обучением.
🥈 2 место на районном научном конкурсе

🎮 МТБанк Хакатон
React, TypeScript, игровая механика
Онлайн-игра для аудитории 18–25 лет. Лидер команды.
